...
5. **Frontend** — `npm --prefix frontend run build` passes; start `bin/dev` and confirm the page loads, the health badge is green, and one chat round-trip renders incrementally.
   - Use Playwright silently for routine UI smoke tests. Prefer a headless shell-driven check such as a one-off Playwright script with `chromium.launch({ headless: true })`, `npx playwright ... --headed=false`, or another non-interactive/headless runner already available in the repo.
   - Do not open the in-app Browser plugin, headed Chromium, Safari, Chrome, or any visible browser window unless the user explicitly asks for visual debugging or a screenshot review.
   - If a screenshot is needed, capture it headlessly to `agent-reports/` and report the path instead of leaving a browser window open.
